<script lang="ts">
	...
	const errorConfigs: Record<string, { icon: string; title: string; description: string }> = {
		expired: {
			...
			title: 'Link Expired',
			description: 'This magic link has expired. Magic links are only valid for a short time for your security.'
		},
		invalid: {
			icon: 'mdi:link-off',
			title: 'Invalid Link',
			description: 'This link is invalid or has already been used. Each magic link can only be used once.'
		},
		email_not_confirmed: {
		...
		access_denied: {
			...
			title: 'Access Denied',
			description: 'You don\'t have permission to access this resource. Please sign in with an authorized account.'
		},
		unknown: {
...
<svelte:head>
	<title>Authentication Error - ChowBenin</title>
</svelte:head>
		...
		<div class="help-section">
			...
			<ul class="help-list">
				<li>
					...
					<span>Request a new magic link from the login page</span>
				</li>
				<li>
					<iconify-icon icon="mdi:clock-outline" width="18" height="18"></iconify-icon>
					<span>Make sure to use the link within a few minutes</span>
				</li>
				<li>
					<iconify-icon icon="mdi:email-check-outline" width="18" height="18"></iconify-icon>
					<span>Check that you're clicking the most recent email</span>
				</li>
			</ul>
		</div>
	</div>
